...
export async function mockListSavTickets() {
	await sleep(250);
	return [...mockDb.tickets].sort((a, b) =>
...
export async function mockCreateSavTicket(input: CreateSavTicketInput) {
	...
	const ticket: SavTicket = {
		...
		slaDueAt: addHours(
			now,
			input.priority === 'urgent' ? 6 : input.priority === 'high' ? 12 : 24
		).toISOString(),
	};
...
export async function mockSetSavTicketStatus(input: {
	id: string;
	status: TicketStatus;
}) {
	await sleep(150);
